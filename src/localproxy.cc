...
void LocalProxy::push(int in_port, Packet * p) {
    ...
    if (in_port == 2) {
        ...
        if ((IDs.size() == 1) && (IDs[0].compare(gc->notificationIID) == 0)) {
            ...
            /*remove the header*/
	    //click_chatter(" i got back an RV/TM event");
	    ...
	    //String print  =  String((const char *) (original->data()), original->length());
	    //click_chatter(print.c_str());
	    ...
	    click_chatter("the packet came from netlink");
	    ...
	    click_chatter("the packet came from click element");
        }
	...
	click_chatter(print.c_str());
	  ...
	  click_chatter("publishing data");
	  //click_chatter("LocalProxy: received request from local application:");
	  ...
	  if (strategy == IMPLICIT_RENDEZVOUS) {
	    ...
	    if ((ID.compare(gc->notificationIID) == 0)) {
	      ...
	      click_chatter("hi");
	      cout<<FID_to_subscribers._data<<endl;
	  ...
	  if (forward) {
	     click_chatter("this is a domain_local and we are publishingReqToRV");
...
void LocalProxy::handleRVNotification(Packet *p) {
    ...
    String nodeID, IP, LID;
    unsigned char IPLength, LIDLength;
    int indexb = 0;

    switch (type) {
        case NEWCONNECTIONS_NODE:
 	    cout<<"new connections"<<endl;
	    output(3).push(p);
	    break;
        case NEWCONNECTIONS_STAR:
 	    cout<<"new connections"<<endl;
	    output(3).push(p);
	    break;
        case RV_RECONFIGURE_NODE:
 	    cout<<"rv_reconfig"<<endl;
	    output(1).push(p);
	    break;
	case RV_RECONFIGURE_RV:
 	    cout<<"rv_reconfig"<<endl;
	    output(1).push(p);
	    break;
...
void LocalProxy::pushDataToRemoteSubscribers(Vector<String> &IDs, BABitvector &FID_to_subscribers, Packet *p) {
    ...
    switch (type) {
	    ...
	    ID = IDs.at(0);
	    numberOfIDs = 1;
	    newPacket = p->push(FID_LEN + sizeof (numberOfIDs) +((int) numberOfIDs) * sizeof (unsigned char) + ID.length());	  
	    memcpy(newPacket->data(), FID_to_subscribers._data, FID_LEN);
	    memcpy(newPacket->data() + FID_LEN, &numberOfIDs, sizeof (numberOfIDs));
	    IDLength = (unsigned char) ID.length() / PURSUIT_ID_LEN;
	    memcpy(newPacket->data() + FID_LEN + sizeof (numberOfIDs), &IDLength, sizeof (IDLength));
	    memcpy(newPacket->data() + FID_LEN + sizeof (numberOfIDs)+ sizeof (IDLength), ID.c_str(), ID.length());
...
void LocalProxy::handleNetworkPublication(Vector<String> &IDs, Packet *p /*the packet has some headroom and only the data which hasn't been copied yet*/) {
    ...
    switch (type) {
        ...
        case RV_RECONFIGURE_RV:
	    	...
	    	break;
        case RV_RECONFIGURE_NODE:
 	    	cout<<"rv_reconfig node"<<endl;
			String newRVFID, newTMFID;
			unsigned char newRVFIDlen, newTMFIDlen;
			newRVFIDlen = *(p->data() + sizeof (type) + sizeof (numberOfIDs);
		    newRVFID  = String((const char *) (p->data())+ sizeof (type) + sizeof (numberOfIDs) + sizeof(newRVFIDlen), newRVFIDlen);
		    newTMFIDlen = *(p->data() + sizeof (type) + sizeof (numberOfIDs) + sizeof() newRVFIDlen + newRVFIDlen);
		    newTMFID = String((const char *) (p->data())+ sizeof (type) + sizeof (numberOfIDs) + sizeof(newRVFIDlen) + newRVFIDlen + sizeof(newTMFIDlen), newTMFIDlen * FID_LEN);

			BABitvector newRVFIDbit = new BABitvector(FID_LEN * 8);
			for (int j = 0; j < newRVFID.length(); j++) {
		   		if (newRVFID.at(j) == '1') {
		       		newRVFIDbit[newRVFID.length() - j - 1] = true;
		   		} else {
		       		newRVFIDbit[newRVFID.length() - j - 1] = false;
	    		}
			}
			gc-> defaultRV_dl = newRVFIDbit;

			BABitvector newTMFIDbit = new BABitvector(FID_LEN * 8);
			for (int j = 0; j < newTMFID.length(); j++) {
		   		if (newTMFID.at(j) == '1') {
		       		newTMFIDbit[newTMFID.length() - j - 1] = true;
		   		} else {
		       		newRMFIDbit[newTMFID.length() - j - 1] = false;
	    		}
			}
			gc->TMFID = newTMFIDbit;
...
void LocalProxy::handleUserPublication(String &ID, BABitvector &FID_to_subscribers, Packet *p, LocalHost *__localhost) {
    ...
    if (localSubscribersSize == 0) {
        /*no need to clone..packet will be sent only to the network*/
        click_chatter("send it off");
        pushDataToRemoteSubscribers(IDs, FID_to_subscribers, p);
    } else {
        click_chatter("local subscriber");
        /*local and remote subscribers exist*/
...
void LocalProxy::publishReqToRV(Packet *p, BABitvector &RVFID) {
	...
	unsigned int len = p2->length();
	unsigned char timeStampLen = ts.length();
	
	WritablePacket *p3 = p2->put(1+timeStampLen);
	memcpy(p3->data() + len, &timeStampLen, sizeof (unsigned char));
        memcpy(p3->data() + len  + sizeof (unsigned char), ts.c_str(), ts.length());
...
void LocalProxy::createAndSendPacketToRV(unsigned char type, unsigned char IDLength /*in fragments of PURSUIT_ID_LEN each*/, String &ID, unsigned char prefixIDLength /*in fragments of PURSUIT_ID_LEN each*/, String &prefixID, BABitvector &RVFID, unsigned char strategy) {
	...
	cout<<ts.c_str()<<endl;
        ...
        p = Packet::make(50, NULL, FID_LEN + sizeof (numberOfIDs) + 1 * sizeof (numberOfFragments) + 2 * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + IDLength * PURSUIT_ID_LEN + sizeof (prefixIDLength) + prefixIDLength * PURSUIT_ID_LEN + sizeof (strategy)+ sizeof(unsigned char)+ ts.length(), 0);
	...
	memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength) + prefixID.length() + sizeof(strategy), &timeStampLength,  sizeof (unsigned char)); 
	memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength) + prefixID.length() + sizeof(strategy) + sizeof(timeStampLength), ts.c_str(), ts.length()); 
