...
int Forwarder::configure(Vector<String> &conf, ErrorHandler *errh) {
    ...
    if (gc->use_mac == true) {
        ...
        cp_integer(conf[1], &number_of_links);
        click_chatter("Forwarder: Number of Links: %d", number_of_links);
	my_src_ip = new IPAddress();
        for (int i = 0; i < number_of_links; i++) {
            cp_integer(conf[2 + 4 * i], &port);
            IPAddress * src_ip = new IPAddress();
            IPAddress * dst_ip = new IPAddress();
	    cp_ip_address(conf[3 + 4 * i], my_src_ip, this);
...
void Forwarder::push(int in_port, Packet *p) {
    ...
    if (in_port == 3){
		...
		String oldLID,failedIP, IP, newLID;
		unsigned char failedIPLength, oldLIDLength, IPLength, LIDLength/* in fragments of FID_LEN*/;
		int indexb = 0;
		failedIPLength = *(p->data() + sizeof (type) + sizeof (numberOfIDs));
		failedIP =  String((const char *) (p->data() + sizeof (type) + sizeof (numberOfIDs) + sizeof (failedIPLength)), failedIPLength);
				
		for (int i=0; i< numberOfIDs; i++){
		    IPLength = *(p->data() + sizeof (type) + sizeof (numberOfIDs) + sizeof (failedIPLength) + failedIPLength+  indexb);
		    IP  = String((const char *) (p->data())+ sizeof (type) + sizeof (numberOfIDs)  + sizeof (failedIPLength) + failedIPLength + sizeof(IPLength) + indexb, IPLength);
		    LIDLength = *(p->data() + sizeof (type) + sizeof (numberOfIDs)  + sizeof (failedIPLength) + failedIPLength + sizeof(IPLength) +IPLength + indexb);
		    newLID = String((const char *) (p->data())+ sizeof (type) + sizeof (numberOfIDs)  + sizeof (failedIPLength) + failedIPLength + sizeof(IPLength) + IPLength + sizeof(LIDLength)+ indexb, LIDLength * FID_LEN);
		
		    //make sure the old connections from failed nodes included doesn't include yourself
		    if (IP!=my_src_ip->unparse()){
		        //TODO get rid of magic numbers-- port should be global var
		        IPAddress * dst_ip = new IPAddress(IP);
				ForwardingEntry *fe = new ForwardingEntry();
				fe->src_ip = my_src_ip;
				fe->dst_ip = dst_ip;
				fe->port = 1;
				fe->LID = new BABitvector(FID_LEN * 8);
				for (int j = 0; j < newLID.length(); j++) {
		    		if (newLID.at(j) == '1') {
		        		(*fe->LID)[newLID.length() - j - 1] = true;
		    		} else {
		        		(*fe->LID)[newLID.length() - j - 1] = false;
		    		}
				}
				fwTable.push_back(fe);
				//automatically forward to new IP if it is not the star node
				if(numberOfIDs==1 && type == NEWCONNECTIONS_NODE){
		  			failedNodes[failedIP] = fe;
				}
	    	}
	  	  	indexb +=  sizeof(IPLength) + IPLength + sizeof(LIDLength) + LIDLength;
		}
    }
    else if (in_port == 0 || in_port == 2) {
        ...
        for (int i = 0; i < fwTable.size(); i++) {
            ...
            if (andVector == (*fe->LID)) {
				if (failedNodes.find(fe->dst_ip->unparse()) == failedNodes.end()){
				    out_links.push_back(fe);
				}else{
				    cout<<"broken link"<<endl;
				    fe = failedNodes[fe->dst_ip->unparse()];
				    out_links.push_back(fe);
				}
            }
        }
        if (out_links.size() == 0) {
	    	...
	    	cout<<"message killed"<<endl;
            p->kill();
        }
		
    	vector<String> outIps;
        for (out_links_it = out_links.begin(); out_links_it != out_links.end(); out_links_it++) {
	  // click_chatter("here in the forwarder");
            ...
            if (gc->use_mac) {
            ...
            } else if (find(outIps.begin(),outIps.end(), fe->dst_ip->unparse()) == outIps.end()){
				outIps.push_back(fe->dst_ip->unparse());
				unsigned char isPing = 0;
	        	WritablePacket *pingPacket = payload->push(sizeof(isPing));
  	        	if(in_port == 2){
		  			isPing = 1;
				}
				memcpy(pingPacket->data(), &isPing, sizeof (unsigned char));
				newPacket = pingPacket->push(sizeof (click_udp) + sizeof (click_ip));   
				...
				if(in_port == 0){
				  ...
				  click_chatter(fe->dst_ip->unparse().c_str());
				}
	    ...
	    isPing = *(p->data() + 28);
	    if (isPing == 1){
	      p->pull(20 + 8 + sizeof(unsigned char) + FID_LEN);
	      output(3).push(p);
	      p->kill();
	    }else{
	      memcpy(FID._data, p->data() + 28 + sizeof(unsigned char), FID_LEN);
	    }
	}
