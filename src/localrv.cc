...
int LocalRV::initialize(ErrorHandler *errh) {
    ...
    masterRV_dl = gc->masterRVFID;
    parentRV_dl = gc->parentRVFID;
    leftRV_dl = gc->leftRVFID;
    rightRV_dl = gc->rightRVFID;
...
void LocalRV::push(int in_port, Packet * p) {
    ...
    unsigned int result;
    unsigned char type, typeOfAPIEvent, IDLengthOfAPIEvent, timeStampLen;
    String IDOfAPIEvent;
    String ID, prefixID;
    String nodeID;
    String timeStamp;
	...
	if (typeOfAPIEvent == PUBLISHED_DATA) {
	    ...
	    if (type == RV_UPDATE){
	      	p->pull(sizeof (typeOfAPIEvent) + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type));
		handle_RV_updates(p);
	    }else{
	      ...
	      timeStampLen = *(p->data() + sizeof (typeOfAPIEvent) + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength) + prefixID.length()+ sizeof(strategy));
	      timeStamp = String((const char *) (p->data() + sizeof (typeOfAPIEvent) + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength)+ prefixID.length()+ sizeof(strategy) + sizeof(timeStampLen)), timeStampLen);
		...
		result = publish_scope(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode(IDLengthOfAPIEvent, IDOfAPIEvent, RV_PUBLISH_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_PUBLISH_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
	      case PUBLISH_INFO:
		...
		result = publish_info(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode(IDLengthOfAPIEvent, IDOfAPIEvent, RV_PUBLISH_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_PUBLISH_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
		...
		result = unpublish_scope(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNPUBLISH_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNPUBLISH_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
	      case UNPUBLISH_INFO:
		...
		result = unpublish_info(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNPUBLISH_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNPUBLISH_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
		...
		result = subscribe_scope(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode(IDLengthOfAPIEvent, IDOfAPIEvent, RV_SUBSCRIBE_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_SUBSCRIBE_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
	      case SUBSCRIBE_INFO:
		...
		result = subscribe_info(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode( IDLengthOfAPIEvent, IDOfAPIEvent, RV_SUBSCRIBE_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes( IDLengthOfAPIEvent, IDOfAPIEvent, RV_SUBSCRIBE_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
		...
		result = unsubscribe_scope(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNSUBSCRIBE_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNSUBSCRIBE_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
		...
		result = unsubscribe_info(_remotehost, ID, prefixID, strategy, false);
		if (result == SUCCESS){
		  UpdateMasterRVNode(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNSUBSCRIBE_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		  UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNSUBSCRIBE_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
		}
		break;
	      case NODE_DISCONNECT:
		click_chatter("LocalRV: node disconnect :%s", nodeID.c_str());
	      default:
	    ...
	    if (result == SUCCESS){
	      if (updateIndex.find(nodeID) == updateIndex.end()){
		// not found
		RVNodeInfo * newNode = new map<String, String>;
		(*newNode)[timeStamp] = ID;
		updateIndex.set(nodeID, newNode);
	      }
	      else {
		RVNodeInfo * newNode = updateIndex.get(nodeID);
		(*newNode)[timeStamp] = ID;
		updateIndex.set(nodeID, newNode);
	      }
	      sendPing(gc->nodeID);
...
unsigned int LocalRV::handle_RV_updates(Packet * p){
    RemoteHost *_remotehost;
    unsigned int result;
    unsigned char type, typeOfAPIEvent;
    unsigned char IDLengthOfAPIEvent, timeLength;
    String IDOfAPIEvent;
    String ID, prefixID, fullID;
    String nodeID;
    String timeStamp;
    unsigned char IDLength/*in fragments of PURSUIT_ID_LEN each*/, prefixIDLength/*in fragments of PURSUIT_ID_LEN each*/, strategy;
    bool hasSeen = true;

    IDLengthOfAPIEvent = *(p->data());
    IDOfAPIEvent = String((const char *) (p->data() + sizeof (IDLengthOfAPIEvent)), IDLengthOfAPIEvent * PURSUIT_ID_LEN);

    nodeID = IDOfAPIEvent.substring(PURSUIT_ID_LEN);
    // click_chatter(nodeID.c_str());
    type = *(p->data() + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN);
    IDLength = *(p->data() + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type));
    ID = String((const char *) (p->data() + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength)), IDLength * PURSUIT_ID_LEN);
    prefixIDLength = *(p->data() + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length());
    prefixID = String((const char *) (p->data()  + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength)), prefixIDLength * PURSUIT_ID_LEN);
    strategy = *(p->data() + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength) + prefixID.length());
    timeLength = *(p->data() + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength) + prefixID.length() + sizeof(strategy));
    timeStamp = String((const char *)(p->data() + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (type) + sizeof (IDLength) + ID.length() + sizeof (prefixIDLength) + prefixID.length() + sizeof(strategy) + sizeof(timeLength)), timeLength);
    ...
    if (updateIndex.find(nodeID) == updateIndex.end()){
      ...
      RVNodeInfo *newNode = updateIndex.get(nodeID);
      /* Vector<string> v;
      for(map<string,String>::iterator it = (*newNode).begin(); it != (*newNode).end(); ++it) {
	v.push_back(it->first);
	cout << it->first.c_str() << "  key"<< endl;
	}*/
      if ((*newNode).find(timeStamp) == (*newNode).end()){
	 (*newNode)[timeStamp] = ID;
	 updateIndex.set(nodeID, newNode);
	 hasSeen = false;
      }
    ...
    if (!hasSeen){
      sendPing(gc->nodeID);
      switch (type) {
      case RV_PUBLISH_SCOPE:
	click_chatter("got an RV update from the master/parent  RV_PUBLISH_SCOPE");
	result = publish_scope(_remotehost, ID, prefixID, strategy, false);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_PUBLISH_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
      case RV_PUBLISH_INFO:
	click_chatter("got an RV update from the master/parent  RV_PUBLISH_INFO");
	result = publish_info(_remotehost, ID, prefixID, strategy, false);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_PUBLISH_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
      case RV_UNPUBLISH_SCOPE:
	click_chatter("got an RV update from the master/parent  RV_UNPUBLISH_SCOPE");
	result = unpublish_scope(_remotehost, ID, prefixID, strategy, false);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNPUBLISH_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
      case RV_UNPUBLISH_INFO:
	click_chatter("got an RV update from the master/parent  RV_UNPUBLISH_INFO");
	result = unpublish_info(_remotehost, ID, prefixID, strategy, false);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNPUBLISH_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
      case RV_SUBSCRIBE_SCOPE:
	click_chatter("got an RV update from the master/parent  RV_SUBSCRIBE_SCOPE");
	if (sub_list.find(nodeID) == sub_list.end()){
	    Vector<String> subList;
	    subList.push_back(SUB_SCOPE + fullID);
	    sub_list[nodeID] = subList;
	}else{
	    sub_list[nodeID].push_back(SUB_SCOPE + fullID);
	}
	usleep(1000);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_SUBSCRIBE_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
      case RV_SUBSCRIBE_INFO:
	click_chatter("got an RV update from the master/parent  RV_SUBSCRIBE_INFO");
	if (sub_list.find(nodeID) == sub_list.end()){
	    Vector<String> subList;
	    subList.push_back(SUB_INFO + fullID);
	    sub_list[nodeID] = subList;
	}else{
	    sub_list[nodeID].push_back(SUB_INFO + fullID);
	}
	usleep(1000);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_SUBSCRIBE_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
      case RV_UNSUBSCRIBE_SCOPE:
	click_chatter("got an RV update from the master/parent RV_UNSUBSCRIBE_SCOPE");
	if (sub_list.find(nodeID) == sub_list.end()){
	    Vector<String> subList;
	    subList.push_back(UNSUB_SCOPE + fullID);
	    sub_list[nodeID] = subList;
	}else{
	    sub_list[nodeID].push_back(UNSUB_SCOPE + fullID);
	}
	usleep(1000);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNSUBSCRIBE_SCOPE, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
      case RV_UNSUBSCRIBE_INFO:
	click_chatter("got an RV update from the master/parent RV_UNSUBSCRIBE_INFO");
	if (sub_list.find(nodeID) == sub_list.end()){
	    Vector<String> subList;
	    subList.push_back(UNSUB_INFO + fullID);
	    sub_list[nodeID] = subList;
	}else{
	    sub_list[nodeID].push_back(UNSUB_INFO + fullID);
	}
	usleep(1000);
	UpdateChildRVNodes(IDLengthOfAPIEvent, IDOfAPIEvent, RV_UNSUBSCRIBE_INFO, IDLength, ID, prefixIDLength,prefixID, strategy, timeStamp);
	break;
...
unsigned int LocalRV::publish_scope(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    ...
    if ((prefixID.length() == 0) && (ID.length() == PURSUIT_ID_LEN)) {
      ret = publish_root_scope(_publisher, ID, strategy, RV_update);
    } else if ((prefixID.length() > 0) && (ID.length() == PURSUIT_ID_LEN)) {
      ret = publish_inner_scope(_publisher, ID, prefixID, strategy, RV_update);
    } else if ((prefixID.length() > 0) && (ID.length() > PURSUIT_ID_LEN)) {
      ret = republish_inner_scope(_publisher, ID, prefixID, strategy, RV_update);
    } else {
        ret = WRONG_IDS;
...
unsigned int LocalRV::publish_root_scope(RemoteHost *_publisher, String &ID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
...
unsigned int LocalRV::publish_inner_scope(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    Scope *sc;
    Scope *fatherScope;
    String fullID;
            ...
            if (sc == scopeIndex.default_value()) {
                ...
                if (fatherScope->strategy == strategy) {
                    ...
                    if (sc->updatePublishers(fullID, _publisher)) {
                        ...
                        click_chatter("LocalRV: added publisher %s to (new) scope: %s(%d)", _publisher->remoteHostID.c_str(), sc->printID().c_str(), (int) strategy);
                        /*notify subscribers here!! only do it if it isn't an RV update*/
...
unsigned int LocalRV::republish_inner_scope(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
                ...
                if (equivalentScope == scopeIndex.default_value()) {
                    if (fatherScope->strategy == strategy) {
                        ...
                        if (existingScope->updatePublishers(fullID, _publisher)) {
                            ...
                            /*notify subscribers here (only if it isn't a RV update)- careful to use the right father as a start!!*/
			    if (!RV_update){
			      RemoteHostSet subscribers;
			      StringSet _ids;
			      fatherScope->getSubscribers(subscribers);
...
unsigned int LocalRV::publish_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    if ((prefixID.length() > 0) && (ID.length() == PURSUIT_ID_LEN)) {
      ret = advertise_info(_publisher, ID, prefixID, strategy, RV_update);
    } else if ((prefixID.length() > 0) && (ID.length() > PURSUIT_ID_LEN)) {
  ret = readvertise_info(_publisher, ID, prefixID, strategy, RV_update);
    } else {
        ret = WRONG_IDS;
...
unsigned int LocalRV::advertise_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    InformationItem *pub;
    Scope *fatherScope;
    ...
    if (fatherScope != scopeIndex.default_value()) {
        ...
        if (pub == pubIndex.default_value()) {
			...
			/* only ask for rendezvous if it isn't a rv updating another rv*/
			if (!RV_update){
			  RemoteHostSet subscribers;
			  pub->getSubscribers(subscribers);
			  click_chatter("trying to match new published info with subscribers");
			  fatherScope->getSubscribers(subscribers);
			  rendezvous(pub, subscribers);
			  click_chatter("finished rendezvous");
			}
                        ret = SUCCESS;
                    } else {
                        ret = EXISTS;
...
unsigned int LocalRV::readvertise_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
...
unsigned int LocalRV::unpublish_scope(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    Scope *sc;
    Scope *fatherScope;
    String fullID = prefixID + ID;
            ...
            if (fatherScope != scopeIndex.default_value()) {
                ...
                for (InformationItemSetIter pub_it = sc->informationitems.begin(); pub_it != sc->informationitems.end(); pub_it++) {
                    ...
                    for (IdsHashMapIter it = sc->ids.begin(); it != sc->ids.end(); it++) {
                        ...
                        unpublish_info(_publisher, pubSuffixID, scPrefixID, strategy, RV_update);
                    }
                }
                ...
                if (pair->first.find(_publisher) != pair->first.end()) {
			     ...
			     if (!sc->checkForOtherPubSub(fatherScope)) {
                                /*notify subscribers about deletion, don't do it if it is rv updating rv*/
			         if (!RV_update){
				   RemoteHostSet subscribers;
				   StringSet _ids;
				   fatherScope->getSubscribers(subscribers);
				 ...
				 for (IdsHashMapIter it = sc->ids.begin(); it != sc->ids.end(); it++) {
				   scopeIndex.erase(it.key());
                            ...
                            if (!sc->checkForOtherPubSub(fatherScope)) {
			      /*notify subscribers about deletion, only if it isn't a RV update from another RV*/
			      if (!RV_update){
				RemoteHostSet subscribers;
...
unsigned int LocalRV::unpublish_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    InformationItem *pub;
    Scope *fatherScope;
    String fullID = prefixID + ID;
            ...
            if (pair->first.find(_publisher) != pair->first.end()) {
                ...
                if (pub->fatherScopes.size() == 1) {
                    if (!pub->checkForOtherPubSub(fatherScope)) {
                        ...
                        /*do the rendezvous again only if it isn't a RV updating this RV*/
			if (!RV_update){
			  RemoteHostSet subscribers;
			  pub->getSubscribers(subscribers);
			  /*careful here...I have multiple fathers*/
...
unsigned int LocalRV::subscribe_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    if ((prefixID.length() == 0) && (ID.length() == PURSUIT_ID_LEN)) {
      ret = subscribe_root_scope(_subscriber, ID, strategy, RV_update);
    } else if ((prefixID.length() > 0) && (ID.length() == PURSUIT_ID_LEN)) {
      ret = subscribe_inner_scope(_subscriber, ID, prefixID, strategy, RV_update);
    } else {
        ret = WRONG_IDS;
...
unsigned int LocalRV::subscribe_root_scope(RemoteHost *_subscriber, String &ID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    Scope *sc = NULL;
    ...
    if (sc == scopeIndex.default_value()) {
        ...
        if (sc->strategy == strategy) {
            if (sc->updateSubscribers(ID, _subscriber)) {
                ...
                /*only update if it isn't a RV updating this RV */
                if (!RV_update){
...
unsigned int LocalRV::subscribe_inner_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    Scope *sc;
    Scope *fatherScope;
    String fullID;
            ...
            if (sc == scopeIndex.default_value()) {
                ...
                if (fatherScope->strategy == strategy) {
                    ...
                    if (sc->updateSubscribers(fullID, _subscriber)) {
                        ...
                        click_chatter("LocalRV: added subscriber %s to (new) scope %s(%d)", _subscriber->remoteHostID.c_str(), sc->printID().c_str(), (int) strategy);
                        /*WEIRD BUT notify other subscribers since the scope has been created!! only if it isn't a RV updating this RV*/
			if(!RV_update){
			  RemoteHostSet subscribers;
			  StringSet _ids;
			  fatherScope->getSubscribers(subscribers);
			  ...
			  notifySubscribers(SCOPE_PUBLISHED, _ids, sc->strategy, subscribers);
                ...
                if (sc->strategy == strategy) {
                    if (sc->updateSubscribers(fullID, _subscriber)) {
                        ...
                        _subscriber->subscribedScopes.find_insert(StringSetItem(fullID));
                        click_chatter("LocalRV: added subscriber %s to scope %s(%d)", _subscriber->remoteHostID.c_str(), sc->printID().c_str(), (int) strategy);
			/*only do this if it isn't a RV updating this RV*/
			if(!RV_update){
			  /*first notify the subscriber about the existing subscopes*/
			  RemoteHostSet subscribers;
			  ScopeSet _subscopes;
...
unsigned int LocalRV::subscribe_info(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    InformationItem *pub;
    Scope *fatherScope;
    if ((prefixID.length() > 0) && (ID.length() == PURSUIT_ID_LEN)) {
            ...
            if (pub == pubIndex.default_value()) {
                ...
                if (fatherScope->strategy == strategy) {
                    if (pub->updateSubscribers(fullID, _subscriber)) {
                        /*add the scope to the publisher's set*/
                        _subscriber->subscribedInformationItems.find_insert(StringSetItem(fullID));
                        /*do the rendez-vous process only if it isn't a RV update from another RV*/
			if(!RV_update){
			  RemoteHostSet subscribers;
			  pub->getSubscribers(subscribers);
			  /*careful here...I have multiple fathers*/
...
unsigned int LocalRV::unsubscribe_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    Scope *sc;
    Scope *fatherScope;
    String fullID = prefixID + ID;
        ...
        if (sc->strategy == strategy) {
            ...
            if (fatherScope != scopeIndex.default_value()) {
                ...
                if (pair->second.find(_subscriber) != pair->second.end()) {
                    ...
                    InformationItemSet _informationitems;
                    sc->getInformationItems(_informationitems);
                    /*then, for each one do the rendez-vous process only if it isn't a RV update from another RV*/
		    if(!RV_update){
...
unsigned int LocalRV::unsubscribe_info(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool RV_update) {
    unsigned int ret;
    InformationItem *pub;
    Scope *fatherScope;
    String fullID = prefixID + ID;
    ...
    if (pub != pubIndex.default_value()) {
        if (fatherScope->strategy == strategy) {
            ...
            if (pair->second.find(_subscriber) != pair->second.end()) {
                ...
                /*do the rendez-vous if there are any left publishers and subscribers and it isn't a RV update from another RV*/
		if(!RV_update){
		  RemoteHostSet subscribers;
		  pub->getSubscribers(subscribers);
		  /*careful here...I have multiple fathers*/
...
void LocalRV::UpdateMasterRVNode(unsigned char IDLengthOfAPIEvent, String &IDOfAPIEvent, unsigned char type, unsigned char IDLength, String &ID, unsigned char prefixIDLength, String &prefixID, unsigned char strategy, String &ts) {

    /*Publish a update to the Master RV*/
    WritablePacket *p,*p1;
    /********FOR THE API*********/
    unsigned char typeForAPI = RV_UPDATE;  
    unsigned char numberOfIDs = 1;
    unsigned char numberOfFragments = 2;
    unsigned char strategyAPI = IMPLICIT_RENDEZVOUS;
    /****************************/
 
    unsigned char timeStampLength = ts.length();

    p = Packet::make(50, NULL, FID_LEN + sizeof (numberOfIDs) + 1 * sizeof (numberOfFragments) + 2 * PURSUIT_ID_LEN + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length() + sizeof(strategy) + sizeof(unsigned char)+ ts.length(), 50);

    memcpy(p->data(), masterRV_dl._data, FID_LEN);
    ...
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN, &typeForAPI, sizeof (typeForAPI)); 

    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI), &IDLengthOfAPIEvent, sizeof(IDLengthOfAPIEvent)); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent), IDOfAPIEvent.c_str(), IDOfAPIEvent.length()); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length(), &type, sizeof(type)); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type), &IDLength, sizeof(IDLength)); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength), ID.c_str(), ID.length()); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length(), &prefixIDLength, sizeof (prefixIDLength)); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength), prefixID.c_str(), prefixID.length()); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length(), &strategy, sizeof(strategy)); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length() + sizeof(strategy), &timeStampLength,  sizeof (unsigned char)); 
    memcpy(p->data() + FID_LEN + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length() + sizeof(strategy) + sizeof (timeStampLength), ts.c_str(), ts.length()); 
    output(1).push(p);
}

/*everything should be sent to the local proxy using the blackadder API*/
void LocalRV::UpdateChildRVNodes(unsigned char IDLengthOfAPIEvent, String &IDOfAPIEvent, unsigned char type, unsigned char IDLength, String &ID, unsigned char prefixIDLength, String &prefixID, unsigned char strategy, String &ts) {
        
    /*Publish a update to the Master RV*/
    int packet_len;
    WritablePacket *p,*p1,*p2;
    /********FOR THE API*********/
    unsigned char typeForAPI = RV_UPDATE;  
    unsigned char numberOfIDs = 1;
    unsigned char numberOfFragments = 2;
    ...
    unsigned char timeStampLength = ts.length();
    p = Packet::make(50 + FID_LEN, NULL, sizeof (numberOfIDs) + 1 * sizeof (numberOfFragments) + 2 * PURSUIT_ID_LEN + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length() + sizeof(strategy)+ sizeof (unsigned char) + timeStampLength, 50);

    memcpy(p->data(), &numberOfIDs, sizeof (unsigned char));
    memcpy(p->data() + sizeof (unsigned char), &numberOfFragments, sizeof (unsigned char));
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char), gc->nodeRVScope.c_str(), 2 * PURSUIT_ID_LEN);
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN, &typeForAPI, sizeof (typeForAPI)); 

    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI), &IDLengthOfAPIEvent, sizeof(IDLengthOfAPIEvent)); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent), IDOfAPIEvent.c_str(), IDOfAPIEvent.length()); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length(), &type, sizeof(type)); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type), &IDLength, sizeof(IDLength)); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength), ID.c_str(), ID.length()); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length(), &prefixIDLength, sizeof (prefixIDLength)); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength), prefixID.c_str(), prefixID.length()); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length(), &strategy, sizeof(strategy)); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length() + sizeof(strategy), &timeStampLength,  sizeof (unsigned char)); 
    memcpy(p->data() + sizeof (unsigned char) + sizeof (unsigned char) + 2 * PURSUIT_ID_LEN  + sizeof (typeForAPI) + sizeof(IDLengthOfAPIEvent)+ IDOfAPIEvent.length() + sizeof(type) + sizeof(IDLength) + ID.length() + sizeof(prefixIDLength) + prefixID.length() + sizeof(strategy) + sizeof (unsigned char), ts.c_str(),  ts.length()); 
	
    p1 = p->clone()->uniqueify();
    p1->push(FID_LEN);
    memcpy(p1->data(), leftRV_dl._data, FID_LEN);
    output(1).push(p1);

    p2 = p->clone()->uniqueify();
    p2->push(FID_LEN);
    memcpy(p2->data(), rightRV_dl._data, FID_LEN);
    output(1).push(p2);
...
void LocalRV::requestTMAssistanceForRendezvous(InformationItem *pub, RemoteHostSet &_publishers, RemoteHostSet &_subscribers, IdsHashMap &IDs) {
    click_chatter("requesting TM assistance for rendezvous");
...
void LocalRV::requestTMAssistanceForNotifyingSubscribers(unsigned char request_type, StringSet &IDs, RemoteHostSet &_subscribers, unsigned char strategy) {
    ...
    for (RemoteHostSetIter iter = _subscribers.begin(); iter != _subscribers.end(); iter++) {
      //sendPing((*iter)._rhpointer->remoteHostID);
...
void LocalRV::sendPing(String nodeID){
    WritablePacket *p;
    p = Packet::make(30, NULL, nodeID.length(), 0);
    memcpy(p->data(), nodeID.c_str(),  nodeID.length());
    output(2).push(p);
}
