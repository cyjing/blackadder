...
void GraphRepresentation::buildRVIGraphTopology() {
    int src_index, dst_index;
    int source_vertex_id, destination_vertex_id;
    igraph_i_set_attribute_table(&igraph_cattribute_table);
    igraph_empty(&igraphRV, 0, true);

	cout<<"starting to build map"<<endl;

    for (int j = 0; j < dm->RV_connections.size(); j++) {
        NetworkConnection *nc = dm->RV_connections[j];
        map <string, int>::iterator index_it;
        index_it = reverse_RVnode_index.find(nc->src_label);
        if (index_it == reverse_RVnode_index.end()) {
            source_vertex_id = igraph_vcount(&igraphRV);
            igraph_add_vertices(&igraphRV, 1, 0);
            reverse_RVnode_index.insert(pair<string, int>(nc->src_label, source_vertex_id));
            igraph_cattribute_VAS_set(&igraphRV, "NODEID", source_vertex_id, nc->src_label.c_str());
        } else {
            source_vertex_id = (*index_it).second;
        }
        index_it = reverse_RVnode_index.find(nc->dst_label);
        if (index_it == reverse_RVnode_index.end()) {
            destination_vertex_id = igraph_vcount(&igraphRV);
            igraph_add_vertices(&igraphRV, 1, 0);
            reverse_RVnode_index.insert(pair<string, int>(nc->dst_label, destination_vertex_id));
            igraph_cattribute_VAS_set(&igraphRV, "NODEID", destination_vertex_id, nc->dst_label.c_str());
        } else {
            destination_vertex_id = (*index_it).second;
        }

	    cout<<source_vertex_id;
	  	cout<<destination_vertex_id<<endl;
        igraph_add_edge(&igraphRV, source_vertex_id, destination_vertex_id);
        igraph_cattribute_EAS_set(&igraphRV, "LID", igraph_ecount(&igraphRV) - 1, nc->LID.to_string().c_str());
    }

	for (int i = 0; i < dm->network_nodes.size(); i++) {
        NetworkNode *nn = dm->network_nodes[i];
		map <string, int>::iterator index_it;
		index_it = reverse_RVnode_index.find(nn->label);
        if (index_it == reverse_RVnode_index.end()) {
            source_vertex_id = igraph_vcount(&igraphRV);
            igraph_add_vertices(&igraphRV, 1, 0);
            reverse_RVnode_index.insert(pair<string, int>(nn->label, source_vertex_id));
            igraph_cattribute_VAS_set(&igraphRV, "NODEID", source_vertex_id, nn->label.c_str());
        }else{
			source_vertex_id = (*index_it).second;
		}
		igraph_cattribute_VAS_set(&igraphRV, "iLID", source_vertex_id, nn->iLid.to_string().c_str());

        map <string, vector<string> * >::iterator indexNode;
		indexNode = RV_nodeList.find(nn->label); 
        if (indexNode != RV_nodeList.end()) {
			vector<string> * nodeList =  indexNode->second;
			string nodes = "";
			for (int j = 0; j < nodeList->size(); j++){
				nodes += nodeList->at(j);
				nodes += ",";
			}			
			igraph_cattribute_VAS_set(&igraphRV, "netWork Node", source_vertex_id, nodes.c_str());
		}
    }
}
...
/*
        NetworkNode *nn = dm->network_nodes[i];
		string RVLabel = dm->getRVNodeLabel();
		NetworkNode *nn2 = dm->findNode(RVLabel);
		cout<<"Rv label of node  " + nn->label + " to rv node of " + RVLabel<<endl;
		nn->RVlabel = RVLabel;
        nn->FID_to_RV = calculateFID(nn->label, RVLabel);

		map <string, vector<string> * >::iterator index_it;
		index_it = RV_nodeList.find(RVLabel); 
        if (index_it == RV_nodeList.end()) {
			vector<string> * newNodeList = new vector<string>;
			newNodeList->push_back(nn->label);
			RV_nodeList[RVLabel] = newNodeList;
        }else{
			vector<string> * nodeList =  index_it->second;
			nodeList->push_back(nn->label);
		}
*/
    }
}
...
void GraphRepresentation::calculateRVNodesFIDs(RV_leaf* rvNode){
	NetworkNode* nn = rvNode->self_RV;
	nn->FID_to_Master_RV = calculateFID(nn->label, dm->Master_RV_node->label);
	nn->FID_to_Parent_RV = calculateFID(nn->label, rvNode->parent_RV->label);	
	

	if (rvNode->left_RV!=NULL){
		nn->FID_to_Left_RV = calculateFID(nn->label, rvNode->left_RV->self_RV->label);
		calculateRVNodesFIDs(rvNode->left_RV);		
	}
	if (rvNode->right_RV!=NULL){
		nn->FID_to_Right_RV = calculateFID(nn->label, rvNode->right_RV->self_RV->label);	
		calculateRVNodesFIDs(rvNode->right_RV);	
	}
}

void GraphRepresentation::calculateRVNodesRecursive(){
	calculateRVNodesFIDs(dm->master);
}
