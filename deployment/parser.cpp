            ...
            for (int role_counter = 0; role_counter < number_of_roles; role_counter++) {
                ...
                if (role.compare("RV") == 0) {
                    ...
                    if (dm->RV_node == NULL) {
                        ...
                        dm->RV_node = nn;
						//making first rv node master, will change this 
						dm->Master_RV_node = nn;
						RV_leaf* masterRoot = new RV_leaf(nn,nn,nn);
						dm->master = masterRoot;
						dm->RV_nodes.push_back(nn);
					} else {
						dm->RV_nodes.push_back(nn);
						dm->master->add(nn, dm->RV_connections);
						cout<<"another rv"<<endl;
                    }
                }
        ...
        if (number_of_roles == 0) {
            ...
            for (int role_counter = 0; role_counter < number_of_roles; role_counter++) {
                ...
                if (role.compare("RV") == 0) {;
                    nn->isRV = true;
                    if (dm->RV_node == NULL) {
                        cout << "node  is the master RV node" << endl;
                        dm->RV_node = nn;
						dm->Master_RV_node = nn;
						RV_leaf* masterRoot = new RV_leaf(nn,nn,nn);
						dm->master = masterRoot;
