...
Domain::Domain() {
    ...
    // to help decide which rv to assign the network node to
    RV_number = 1;
}
...
void Domain::writeClickFiles(bool montoolstub) {
	NetworkNode *nn = Master_RV_node;
	RV_leaf *rvTree = new RV_leaf(Master_RV_node,  Master_RV_node,  Master_RV_node);
	for (int i = 1; i < RV_nodes.size(); i++ ){
	     NetworkNode *nn = RV_nodes.at(i);
	     cout<<"RV" + nn->label <<endl;
	}
    ofstream click_conf;
    for (int i = 0; i < network_nodes.size(); i++) {
	...
	click_conf << "ping::Ping(globalconf," << nn->connections.size() /*number of neighbours*/ << "," << endl;
        for (int j = 0; j < nn->connections.size(); j++) {
            NetworkConnection *nc = nn->connections[j];
            NetworkNode *dest_nn = findNode(nc->dst_label);
            if (j == nn->connections.size() - 1) {
