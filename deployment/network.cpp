...
void Domain::printDomainData() {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        cout << "testbed_ip " << nn->testbed_ip << endl;        cout << "iLid " << nn->iLid.to_string() << endl;
...
void Domain::assignLIDs() {
	...
	cout<<RV_connections.size();

    int totalLIDs = number_of_nodes/*the iLIDs*/ + number_of_connections + RV_connections.size();
    ...
    for(int i = 0; i< RV_connections.size(); i++){
	NetworkConnection *nc = RV_connections[i];
        nc->LID = LIDs[LIDCounter];
        LIDCounter++;
    }
...
void Domain::writeClickFiles(bool montoolstub) {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
		...
		click_conf<<"masterRVFID "<<nn->FID_to_Master_RV.to_string()<< "," <<endl;
		click_conf<<"parentRVFID "<<nn->FID_to_Parent_RV.to_string()<< "," <<endl;
		click_conf<<"leftRVFID   "<<nn->FID_to_Left_RV.to_string()<< "," <<endl;
		click_conf<<"rightRVFID  "<<nn->FID_to_Right_RV.to_string()<< "," <<endl;
...
string Domain::getRVNodeLabel(){
  string label = "";

  if (RV_nodes.size()==0){
    //error, there must be at least one RV node
  }else{
    int rvPos = RV_number%(RV_nodes.size());
    label = RV_nodes.at(rvPos)->label;
  }
...
void RV_leaf::add(NetworkNode* newRV, vector<NetworkConnection *> &RV_connections){
  if (left_RV==NULL){
    left_RV = new RV_leaf(master_RV, self_RV, newRV);
	NetworkConnection *nc = new NetworkConnection();
    nc->src_label = newRV->label;
    nc->dst_label = self_RV->label;
    nc->src_ip = newRV->testbed_ip;
    nc->dst_ip = self_RV->testbed_ip;
    RV_connections.push_back(nc);
	NetworkConnection *nc2 = new NetworkConnection();
    nc2->src_label = self_RV->label;
    nc2->dst_label = newRV->label;
    nc2->src_ip = self_RV->testbed_ip;
    nc2->dst_ip = newRV->testbed_ip;
    RV_connections.push_back(nc2);
  }else if (right_RV==NULL){
    right_RV = new RV_leaf(master_RV, self_RV, newRV);
	NetworkConnection *nc = new NetworkConnection();
    nc->src_label = newRV->label;
    nc->dst_label = self_RV->label;
    nc->src_ip = newRV->testbed_ip;
    ...
    if (leftNodes<=rightNodes){
      left_RV->add(newRV, RV_connections);
    }else{
      right_RV->add(newRV, RV_connections);
    }
  }
}
