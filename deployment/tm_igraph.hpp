...
public:
    ...
    /**@brief reads the RVgraphml file that describes the network topology and creates some indexes for making the operation faster.
     * 
     * Currently iGraph cannot read the Global Graph variables and, therefore, this methid does it manually.
     * 
     * @param name the /graphML file name
	...
	/**@brief add a new node to igraph to reconfigure after a
	*  RV has failed
	*/
	void addToRVIgraph(long int e);
