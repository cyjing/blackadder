...
public:
    ...
    int readRVTopology(char *file_name);  

    void failedNodeReconfig(string & node);

    string failedRVReconfig(string & RVNode);
	...
	void createRVFailureMessages(RV_leaf * node, vector<string> &localNodes);

  //  void addRVUpdateMessage(int lenofResponse, unsigned char response_type);
    ...
    /**@brief an index that maps LID labels to igraph edge ids.
     */
     ...
     */
    unsigned char fid_len;
    /**@brief mode in which this Blackadder node runs - the TM must create the appropriate netlink socket.
     */
    string mode;

    map<string, string> nodeID_IP;
    map<string, RV_leaf *> RVNodeMap;
    RV_leaf * master;
    map<int, string> rv_node_index;
    map<string, int> rv_reverse_node_index;

    map<int, string> node_index;
  /**@brief map of RV node with leaf nodes (non-RV nodes that subscribe to that RV node)
   */
    map<string, vector<string> > nodeLists;
    vector<pair<pair<string, int> , char *> > whatImReturning;
};
